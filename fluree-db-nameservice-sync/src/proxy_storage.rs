...
fn cid_and_ledger_from_address(address: &str) -> Option<(ContentId, String)> {
    ...
    // Determine the multicodec and where the alias ends based on directory
    // structure. These patterns match those generated by `content_path()` in
    // fluree-db-core; `branch_in_path` is false for the @shared dict layout,
    // whose addresses carry only the ledger name.
    let (codec, alias_end, branch_in_path) = if n >= 2 && parts[n - 2] == "commit" {
        (CODEC_FLUREE_COMMIT, n - 2, true)
    } else if n >= 2 && parts[n - 2] == "txn" {
        (CODEC_FLUREE_TXN, n - 2, true)
    } else if n >= 2 && parts[n - 2] == "config" {
        (CODEC_FLUREE_LEDGER_CONFIG, n - 2, true)
    } else if n >= 3 && parts[n - 3] == "index" && parts[n - 2] == "roots" {
        (CODEC_FLUREE_INDEX_ROOT, n - 3, true)
    } else if n >= 3 && parts[n - 3] == "index" && parts[n - 2] == "garbage" {
        (CODEC_FLUREE_GARBAGE, n - 3, true)
    } else if n >= 4 && parts[n - 4] == "index" && parts[n - 3] == "objects" {
        match parts[n - 2] {
            "branches" => (CODEC_FLUREE_INDEX_BRANCH, n - 4, true),
            "leaves" => (CODEC_FLUREE_INDEX_LEAF, n - 4, true),
            // Legacy per-branch dict layout (pre-@shared):
            // {name}/{branch}/index/objects/dicts/{hash}.dict
            "dicts" => (CODEC_FLUREE_DICT_BLOB, n - 4, true),
            _ => return None,
        }
        ...
        (CODEC_FLUREE_DICT_BLOB, n - 3, false)
    } else {
        return None;
    };
