...
| `UNWIND` | ✅ | Inline lists and runtime list expressions; `$param` lists via API substitution. Before a **write** clause: `$param` lists, inline literal lists, and constant `range()` (≤ 10000 rows). A `$param` list of maps before a `MATCH…CREATE` (batched edge insert) or a node `MERGE` (batched node upsert) desugars to a VALUES join. |
...
| `MERGE` (node) | ✅ | Find-or-create with `ON CREATE SET` / `ON MATCH SET`, and trailing `SET` clauses that apply on both branches — the upsert idiom `MERGE (n:User {id: $id}) SET n += $props` works. **Per-row** node upsert via `UNWIND $batch AS row MERGE (n {id: row.id}) SET …` (the `LOAD CSV` shape, powering `fluree load`): the batch desugars to a VALUES join, one NOT EXISTS guard per row, `ON MATCH`/trailing SET staged atomically with the create branch. A leading *plain* `MATCH` before a node MERGE stays rejected — a row-independent key would create one duplicate per matched row. |
| `MERGE` (relationship) | ◑ | Standalone + bound-endpoint forms; property-bearing patterns (`-[:T {p:v}]->` matches on the annotation values — a different value creates a parallel edge); `ON CREATE SET` on endpoint and relationship variables; `ON MATCH SET` / trailing `SET` on both the **standalone** form (probe-then-branch) and the **per-row** form (leading `MATCH`), the latter staging an `ON MATCH SET` branch and a create branch into one atomic commit. Deferred: multi-hop / multi-part MERGE, multiple `MERGE` clauses, and combining a MERGE with a non-`SET` write clause. |
...
| `LOAD CSV` | ✅ (CLI) | Not a query-language clause; served by the CLI's [`fluree load`](../cli/load.md), which reads the CSV client-side and streams it as batched per-row upserts — `--cypher` (wrapped in `UNWIND $batch AS row …`) or `--jsonld` (batch injected as the update's `values` clause). Bulk one-shot import of a fresh dataset is separate: `fluree create --from *.csv` / `*.cypher`. |
